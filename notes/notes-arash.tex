...
% I'm getting tired of writin \otimes, lets define some macros ... /Arash
...
\begin{minipage}{4.5in}
\begin{tt}
{\bf Algorithm: Learning the parameters}

\begin{enumerate}
  \item
    Let $S$ be the samples, it's a matrix with $m$ rows of points in $\R^n$. In
    this algorithm context, we let $Y_E$ be $\frac{1}{m} \sum_i^m S_{(i)}$.
    Similarly $Y_{E^2} = \frac{1}{m} \sum_i^m S_{(i)} \otimes S_{(i)}$. Where
    $S_{(i)}$ is the $i$th row of $S$ as a column vector.
  \item Get any of the  $n-k$ smallest singular values of $Y_{E^2}$, they should
    all be the same and equal to $\sum_i^k w_i \sigma_i^2$
  \item Form $B_2 = Y_{E^2} - (\sum_i w_i \sigma_i^2)I$ and decompose it into $B_2 = W W^T$.
    Now form $\tilde Y_{E^3} = \frac{1}{m} \sum_i^m \tensorsame{W^{-1} S_{(i)}}$
  \item Form $\tilde B_3 = \tilde Y_{E^3} - \sum_j^n \perms{\tilde u_E}{e_j}$
    where $\tilde u_E = \frac{1}{m} \sum_i^m W^{-1}
    S_{(i)}\left((W^{-1} S_{(i)}-\tilde Y_E) \cdot e_1 \right)^2$
  \item Let a random unit vector $x$
    in $\R^n$ converge when applying a projected power iteration on
    $\tilde B_3$, that is repeteadly applying $x := normalize(P x^T \tilde B_3 x)$
    until convergence where $P$ is $I$ initially. Then set $\mu_1 =
    \sum_{jkl} \left(\tilde B_{3_{jkl}} x_j x_k x_l \right) x$ and $P := P P_{\perp \tilde
      \mu_1} = P (I - \frac{\tilde \mu_1 \tilde \mu_1^T}{\tilde \mu_1^T \tilde
      \mu_1})$. Also $w_1 = \frac{1}{|\tilde \mu_1|^2} $. Now repeat
    this procedure to unveil $\tilde \mu_2 \dots \tilde \mu_k$.
  \item Even though knowing all weights and each $\mu_i$ (as of $\mu_i = W \tilde \mu_i$).
    There is no closed formula for retrieving the variances.  However,
    we can easily guess the origins of the points. Knowing the
    means, we can simply sort the points by shortest distance to any mean.  Also, we
    know approximately when a distribution has gotten assigned enough points
    since we have the weights for all distributions too.
\end{enumerate}
\end{tt}
\end{minipage}
}}
