...
tensor product, a matrix times a vector, say $A \otimes u$ is a tensor
with three dimensions. That is the resulting dimensionality is the sum
of the arguments dimensionality.

The second notation is that $X_i$ refers to the distribution $ X_i \sim
N(\mu_i, \sigma_i^2) $ but $Y_j$ simply is the $j$:th component of $Y$.  $i$
will always loop from $1$ to $k$ but $j$, $k$ and $l$ will loop from $1$ to
$n$. Lastly, there is no $X$ without any index, nor does $\mu$ and $\sigma$,
but we really mean $X_i$, $\mu_i$ etc. for all $i$ when we use them without a
index.
...
One interesting idea is to take $v$ such that $v \perp span\{\mu_1 \dots
\mu_k\}$, then we have:
...
Of course we don't know the $\mu_i$s so finding that $v$ orthogonal to them is
not trivial but possible. (\textbf{is this true??????})

Now we'll do the analogous calculations for $X \otimes X \otimes X$ and $Y$.

\begin{eqnarray*}
  \E(X \otimes X \otimes X)
  &=& \dots \\
  &=& \mu \otimes \mu \otimes \mu + \sigma_i^2 \sum_j (e_j \otimes e_j \otimes \mu + \dots)
\end{eqnarray*}
...
By now, we've established equations that let us state the main theorem of
this section.

\begin{theorem}\label{thm:learnmix}
  Given $B_2$ and $B_3$ from above. If the means are linearly independent, we
  can find all the three parameters for each distribution in polynomial time.
\end{theorem}

Make $B_2$ isotropic by decomposing it into $B_2 = W W^T$. Let $\tilde{\mu_i} = W^{-1} \mu_i$.
...
Seeing $T$ as a function we'll get
